...
import (
	...
	notifyservice "github.com/mouse233/MySekaiMapper/internal/notify"
	httpserver "github.com/mouse233/MySekaiMapper/internal/server"
...
func runGenerate(root string, args []string) {
	...
	input := flags.String("input", "", "encrypted .bin save path (required)")
	output := flags.String("output", filepath.Join(root, "data", "latest"), "output directory")
...
func runNotify(root string, args []string) {
	flags := flag.NewFlagSet("notify", flag.ExitOnError)
	output := flags.String("output", "", "directory containing site_*.png and rare_resources.txt (required)")
	taskID := flags.String("task-id", "unknown", "task identifier for the notification")
	playerID := flags.String("player-id", "unknown", "player ID used for push routing")
	imageBase := flags.String("image-base", "", "public image URL base for Bark")
	envFile := flags.String("env", filepath.Join(root, ".env"), "dotenv file path")
	_ = flags.Parse(args)
	...
	if err := notifier.Notify(context.Background(), *output, *taskID, *playerID, *imageBase); err != nil {
		fatal(err)
	}
	fmt.Printf("[DONE] Notification dispatch complete for task %s player_id=%s\n", *taskID, *playerID)
}
...
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: mysekaimapper <inspect|generate|notify|serve> [--root <repository-root>] [options]")
	...
	fmt.Fprintln(os.Stderr, "  notify   send existing map PNGs and rare-resource statistics")
