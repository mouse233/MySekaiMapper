...
// for the JSON forms accepted by push_map.json.
...
func (n *Notifier) Notify(ctx context.Context, outputDir, taskID, playerID, imageBase string) error {
	...
	images := siteImagePaths(outputDir)
	...
	for _, alias := range barkAliases {
		...
		for _, imagePath := range images {
			...
			imageURL := ""
			if base != "" {
				imageURL = base + "/" + url.PathEscape(filename)
			}
			title := "site" + strings.TrimSuffix(strings.TrimPrefix(filename, "site_"), filepath.Ext(filename))
			if err := n.sendBark(ctx, key, title, "", imageURL, n.config.BarkIcon); err != nil {
				errorsSeen = append(errorsSeen, err)
			}
		}
	}
...
// siteImagePaths returns every regular site_*.png output in lexical order.
// It deliberately uses Lstat so an attacker cannot redirect a notification upload
// through a symlink placed in the output directory.
func siteImagePaths(outputDir string) []string {
	matches, err := filepath.Glob(filepath.Join(outputDir, "site_*.png"))
	if err != nil {
		return nil
	}
