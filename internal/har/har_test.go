...
func zstdBodyWithoutContentSize(t *testing.T, plain []byte) []byte {
	t.Helper()
	var output bytes.Buffer
	...
	writer.ResetContentSize(&output, -1)
	if _, err := writer.Write(plain); err != nil {
		t.Fatal(err)
	}
...
func TestDecompressBodySupportsZstdWithoutContentSize(t *testing.T) {
	plain := bytes.Repeat([]byte("hello zstd no-size"), 1000)
	raw := zstdBodyWithoutContentSize(t, plain)
	if len(raw) < 5 {
		t.Fatalf("zstd frame too short: %d bytes", len(raw))
	}
	// In the frame header descriptor, a content-size field is present when the
	// two high bits are non-zero, or when the single-segment bit is set.
	if descriptor := raw[4]; descriptor&0xc0 != 0 || descriptor&0x20 != 0 {
		t.Fatalf("zstd frame unexpectedly has content size: descriptor=%#02x", descriptor)
	}
	got, err := DecompressBody(raw, "zstd", int64(len(plain)+1))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, plain) {
