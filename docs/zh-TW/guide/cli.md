...
- 啟動 FastAPI 服務：用戶端向 `POST /uploadMySekai` 分片上傳加密存檔（介面細節見[上傳介面](/zh-TW/guide/upload-api)）；Reqable 也可把 HAR 工作階段上報到內建上報端點（見[Reqable 上報伺服器](/zh-TW/guide/report-server)）
